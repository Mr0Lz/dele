<template>
    
    <section class="wrapper" ref="wrapper" >
        <div class="content">
          <slot></slot>
          <!-- 明天继续 得放到.content 里面 加载更多的时候才会在最末尾  -->
          <slot name="pullup"
                :pullUpLoad="pullUpLoad"
                :isPullUpLoad="isPullUpLoad">
            <div class="pullup-wrapper"
                v-if="pullUpLoad">
              <div class="before-trigger"
                  v-if="!isPullUpLoad">
                <span>{{pullUpTxt}}</span>
              </div>
              <div class="after-trigger"
                  v-else>
                <img class="scroll-loading"
                    src="data:image/gif;base64,">
              </div>
            </div>
          </slot>

        </div>
      <!-- 更多数据 -->

        <!-- 下拉更新 -->
        <slot name="pulldown"
              :pullDownRefresh="pullDownRefresh"
              :pullDownStyle="pullDownStyle"
              :beforePullDown="beforePullDown"
              :pulling="pulling"
              :bubbleY="bubbleY">
          <div ref="pulldown"
              class="pulldown-wrapper"
              :style="pullDownStyle"
              v-if="pullDownRefresh">
            <div class="before-trigger"
                v-if="beforePullDown">
              <bubble :y="bubbleY"></bubble>
            </div>
            <div class="after-trigger"
                v-else>
              <div v-if="pulling"
                  class="loading">
                <img class="scroll-loading"
                    src="data:image/gif;base64,">
              </div>
              <div v-else><span>{{refreshTxt}}</span></div>
            </div>
          </div>
        </slot>
    </section>

</template>

<script>
import BScroll from 'better-scroll';
import bubble from '@/components/canvasAnimation/dragBubble';
//滑动方向
const DIRECTION_H = 'horizontal'
const DIRECTION_V = 'vertical'

export default {
    name : 'scroll',
    components: {
      bubble
    },
    data() {
      return {
        beforePullDown: true,//控制气泡动画
        isRebounding: false,//回弹中
        isPullingDown: false,
        pulling: false,
        isPullUpLoad: false,
        pullUpDirty: true, // 此变量用来检查是否加载到了最后一页  false 没有更多
        pullDownStyle: '',//.pulldown-wrapper  top样式
        bubbleY: 0,
        reboundPullDownTimer: null,
        afterPullDownTimer: null
      }
    },
    props:{
        width:{
            type: Number
        },
        height:{
            type: Number
        },
        // 作用：有时候我们需要知道滚动的位置。当 probeType 为 1 的时候，会非实时（屏幕滑动超过一定时间后）派发scroll 事件；当 probeType 为 2 的时候，会在屏幕滑动的过程中实时的派发 scroll 事件；当 probeType 为 3 的时候，不仅在屏幕滑动的过程中，而且在 momentum 滚动动画运行过程中实时派发 scroll 事件。如果没有设置该值，其默认值为 0，即不派发 scroll 事件。
      probeType: {
        type: Number,
        default: 1
      },
      //作用：better-scroll 默认会阻止浏览器的原生 click 事件。
      click: {
        type: Boolean,
        default: true
      },
      //scroll事件 参数：   {Object} {x, y} 滚动的实时坐标 触发时机：滚动过程中，具体时机取决于选项中的 probeType。
      listenScroll: {
        type: Boolean,
        default: false
      },
      //beforeScrollStart事件 参数：无  触发时机：滚动开始之前。
      listenBeforeScroll: {
        type: Boolean,
        default: false
      },
      //scrollX ,scrollY当设置为 true 的时候，可以开启横/纵向滚动。
      direction: {
        type: String,
        default: DIRECTION_V
      },
      // scrollbar: {fade: true,interactive: false // 1.8.0 新增}fade 为 true 表示当滚动停止的时候滚动条是否需要渐隐，interactive 表示滚动条是否可以交互。
      scrollbar: {
        type: null,
        default: false
      },
      //pullDownRefresh: {threshold: 50,stop: 2}可以配置顶部下拉的距离（threshold） 来决定刷新时机以及回弹停留的距离（stop）。当下拉刷新数据加载完毕后，需要执行 finishPullDown 方法
      pullDownRefresh: {
        type: null,
        default: false
        //default:true
      },
      //pullUpLoad: {threshold: 50}可以配置离（threshold）来决定开始加载的时机。当上拉加载数据加载完毕后，需要执行 finishPullUp 方法
      pullUpLoad: {
        type: null,
        default: false
      },
      //纵轴方向初始化位置
      startY: {
        type: Number,
        default: 0
      },
      refreshDelay: {
        type: Number,
        default: 20
      },
      //作用：有些场景我们需要支持横向和纵向同时滚动
      freeScroll: {
        type: Boolean,
        default: false
      }
    },
    computed: {
      pullUpTxt() {
        const moreTxt = (this.pullUpLoad && this.pullUpLoad.txt && this.pullUpLoad.txt.more) || '加载更多'
        const noMoreTxt = (this.pullUpLoad && this.pullUpLoad.txt && this.pullUpLoad.txt.noMore) || '没有更多数据了'
        return this.pullUpDirty ? moreTxt : noMoreTxt
      },
      refreshTxt() {
        return (this.pullDownRefresh && this.pullDownRefresh.txt) || '刷新成功'
      }
    },
    created() {
      //canvas 动画 top 偏移-50 到屏幕外  ,scroll  需要加上
      this.pullDownInitTop = -50;
      this.pullDownStyle = `top:${this.pullDownInitTop}px`;
    },
    mounted(){
        let offsetObj = this.$refs.wrapper.getBoundingClientRect();//获取元素相对于视窗的位置集合。
        let docScrollTop = document.documentElement.scrollTop || document.body.scrollTop; //得加上窗口已经滚动过的高度
        let clientHeight = document.body.clientHeight;
        // let  offsetParent = this.$refs.wrapper.offsetParent ;//定位父元素
        let height = clientHeight - offsetObj.top - docScrollTop;//为元素相对于视窗高度填满剩下的高度
        this.$refs.wrapper.style.height = this.height ? this.height+'px' : height + 'px' ;

        this.$nextTick(() => {  //nextTick 等到元素更新高度后 初始化Scroll
            this.initScroll()
          })
    },
    methods : {
      disable() {
        this.scroll && this.scroll.disable()
      },
      enable() {
        this.scroll && this.scroll.enable()
      },
      refresh() {
        this.scroll && this.scroll.refresh()
      },
      scrollTo() {
        this.scroll && this.scroll.scrollTo.apply(this.scroll, arguments)
      },
      scrollToElement() {
        this.scroll && this.scroll.scrollToElement.apply(this.scroll, arguments)
      },
      destroy() {
        this.scroll && this.scroll.destroy()
      },
      initScroll(){
        if (!this.$refs.wrapper) { //没有获取到wrapper元素  return
          return
        }
        const options = { //初始化 scroll 的第二个参数
          probeType: this.probeType,
          click: this.click,
          scrollY: this.freeScroll || this.direction === DIRECTION_V,
          scrollX: this.freeScroll || this.direction === DIRECTION_H,
          scrollbar: this.scrollbar,
          pullDownRefresh: this.pullDownRefresh,
          pullUpLoad: this.pullUpLoad,
          startY: this.startY,
          freeScroll: this.freeScroll
        }
        this.scroll = new BScroll(this.$refs.wrapper, options)//初始化
        if (this.listenScroll) {//是否需要监听scroll事件
          this.scroll.on('scroll', pos => {
            this.$emit('scroll', pos)
          })
        }

        if (this.listenBeforeScroll) {//是否需要监听滚动前事件
          this.scroll.on('beforeScrollStart', () => {
            this.$emit('beforeScrollStart')
          })
        }
        if (this.pullDownRefresh) { //是否需要PullDownRefresh
          this._initPullDownRefresh()
        }

        if (this.pullUpLoad) {//是否需要pullUpLoad
          this._initPullUpLoad()
        }
      },
      _initPullDownRefresh() {
        //下拉刷新事件
        this.scroll.on('pullingDown', () => {
          this.beforePullDown = false //气泡动画消失
          this.isPullingDown = true //进入下拉刷新状态
          this.pulling = true//出现菊花图
          this.$emit('pullingDown',this)//促发刷新事件
        })

        //滚动事件
        this.scroll.on('scroll', pos => {
          //scroll事件  参数：pos : {x, y} 滚动的实时坐标
          if (this.beforePullDown) {//气泡拖拽动画
            this.bubbleY = Math.max(0, pos.y + this.pullDownInitTop)
            this.pullDownStyle = `top:${Math.min(pos.y + this.pullDownInitTop, 10)}px`
          } else {
            this.bubbleY = 0//气泡动画消失  复位
          }

          if (this.isRebounding) {
            this.pullDownStyle = `top:${10 - (this.pullDownRefresh.stop - pos.y)}px`
          }
        })
      },
      _initPullUpLoad() {
        this.scroll.on('pullingUp', () => {
          if (this.pullUpDirty) {
            this.isPullUpLoad = true
            this.$emit('pullingUp',this)
          }
        })
      },
      // 加载数据后更新
      forceUpdate(dirty) {
        this.$nextTick(() => {
          this.pullUpDirty = dirty
          if (this.pullDownRefresh && this.isPullingDown) {
            this.pulling = false
            this._reboundPullDown().then(() => {
              this._afterPullDown()
            })
          } else if (this.pullUpLoad) {
            this.isPullUpLoad = false
            this.scroll.finishPullUp()
            this.refresh()
          } else {
            this.refresh()
          }
        })
      },
      _reboundPullDown() {
        const { stopTime = 600 } = this.pullDownRefresh
        return new Promise(resolve => {
          this.reboundPullDownTimer = setTimeout(() => {
            this.isRebounding = true
            this.isPullingDown = false
            this.scroll.finishPullUp()//当上拉加载数据加载完毕后，需要调用此方法告诉 better-scroll 数据已加载。
            this.scroll.finishPullDown()//当下拉刷新数据加载完毕后，需要调用此方法告诉 better-scroll 数据已加载。
            resolve()
          }, stopTime)
        })
      },
      _afterPullDown() {
        this.afterPullDownTimer = setTimeout(() => {
          this.pullDownStyle = `top:${this.pullDownInitTop}px`
          this.beforePullDown = true
          this.isRebounding = false
          this.refresh()
        }, this.scroll.options.bounceTime)
      }

    }
}

</script>

<style  lang="scss" scoped>
@import '../../style/mixin';

.wrapper{
  position: relative;
  overflow: hidden;
  .pulldown-wrapper{
    font-size: initial;
    position: absolute;
    width: 100%;
    // top: -50px;
    //top:0px;
    left: 0;
    text-align: center;
    transition: all;
  }
  .pullup-wrapper{
    text-align: center;
    font-size: initial;
  }
  .before-trigger,.after-trigger{
      display: inline-block;
  }
  .scroll-loading{
    width: 20px;
    height: 20px;
  }
}


</style>
